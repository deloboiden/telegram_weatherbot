...
class Sensors():
    ...
    def read_data(self):
        try:
            self.pressure, temperature1 = sensBMP()
            self.humidity, temperature2 = sensDHT()
    ...
    def print_data(self, filename=None):
        if filename is not None:
            ...
            print("temperature: %.2f, pressure: %.2f, humidity: %.2f" % round(self.temperature, 2),
                  round(self.pressure, 2), round(self.humidity, 2))

    def write_to_lcd(self):
        try:
            display.lcd_display_string("pressure: %.2f" % round(self.pressure, 2), 1)
            display.lcd_display_string("temp %.2f Hum %.2f" % round(self.temperature, 2), round(self.humidity, 2), 2)
        except Exception as err:
            print("Error: ", err)
            create_log(err, 2)
